...
const ProblemCard = ({ text, delay }: { text: string; delay: number }) => (
  <motion.div 
    ...
    viewport={{ once: true }}
    transition={{ delay }}
    className="p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 mb-4 group hover:border-white/10 transition-all"
  >
    <div className="flex items-center gap-4">
      <div className="w-1.5 h-1.5 rounded-full bg-white/20 group-hover:bg-red-600 transition-colors" />
      <p className="text-white/40 group-hover:text-white transition-colors text-lg font-light">{text}</p>
    </div>
  </motion.div>
);
...
const ScrollStory: React.FC = () => {
  return (
    <section className="py-60 bg-black overflow-hidden border-y border-white/5">
      <div className="container mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-20 items-center">
          <div>
            <span className="text-red-600 font-mono text-[10px] uppercase tracking-[0.5em] mb-6 block">The Bottleneck</span>
            <h2 className="text-5xl md:text-7xl font-heading font-bold mb-12 tracking-tighter">
              Boring Work <br /> 
              <span className="text-white/30 italic">Kills Growth.</span>
            </h2>
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            className="relative p-12 lg:p-20 rounded-[4rem] bg-[#0c0c0c] border border-white/5 overflow-hidden group"
          >
            <div className="absolute top-0 right-0 w-96 h-96 bg-red-600/5 blur-[120px] pointer-events-none"></div>
            
            <h3 className="text-4xl font-heading font-bold mb-8 text-white tracking-tight">The Dizitup Intelligence</h3>
