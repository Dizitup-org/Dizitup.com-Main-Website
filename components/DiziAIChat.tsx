...
const DiziAIChat: React.FC = () => {
  ...
  // When logged in, ChatWidget handles the Dizi AI experience — don't render here
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-8 right-8 z-40 w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-red-600 to-red-800 shadow-2xl shadow-red-600/50 flex items-center justify-center cursor-pointer hover:shadow-red-600/70 transition-shadow duration-300 border border-red-500/30"
      >
        <motion.div animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.3 }}>
          {isExpanded
            ? <X className="w-7 h-7 sm:w-9 sm:h-9 text-white" />
      ...
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            className="fixed bottom-28 sm:bottom-36 right-6 sm:right-8 z-40 w-80 sm:w-96 rounded-2xl overflow-hidden backdrop-blur-xl border border-white/10 shadow-2xl shadow-black/60"
          >
            <div className="relative bg-gradient-to-b from-[#0d0d0d] to-[#0a0a0a] overflow-hidden">
              {/* Glow */}
              ...
              {!isLoggedIn ? (
                <div className="relative p-5 sm:p-6 space-y-5">
                  ...
                  <motion.button
                    ...
                    className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white font-bold text-sm tracking-wide transition-all shadow-lg shadow-red-600/30 flex items-center justify-center gap-2"
                  >
                ...
                /* ── LOGGED-IN CONTENT ── */
                <div className="relative p-4 sm:p-5 space-y-4 max-h-[380px] overflow-y-auto">
              ...
              {/* Input bar (disabled teaser) */}
              <div className="relative px-4 py-3 border-t border-white/5 flex items-center gap-3">
                <div className="flex-1 px-4 py-2.5 rounded-full bg-white/5 border border-white/10 text-white/20 text-sm select-none">
                  {isLoggedIn ? 'Dizi AI launching soon...' : 'Log in to chat with Dizi...'}
                </div>
                <div className="w-9 h-9 rounded-full bg-red-600/30 flex items-center justify-center flex-shrink-0">
